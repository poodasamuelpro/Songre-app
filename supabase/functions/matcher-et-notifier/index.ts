...
// Plafond de destinataires notifiés par demande (ajouté 2026-07-16), pour
// éviter tout dépassement de quota API (Brevo/Resend/FCM) si une demande
// touche un très grand nombre de donneurs compatibles dans une même ville.
...
// Mis à jour le 2026-07-16 : femme = 120j, homme/autre = 90j (anciennement 90/60j).
// Synchronisé avec retour-eligibilite-cron/index.ts. Reste à synchroniser le
// trigger SQL verifier_eligibilite_donneur() et l'affichage côté Flutter.
...
function estEligible(profil: ProfilDonneur): boolean {
  ...
  const seuilJours = profil.genre === "femme" ? 120 : 90;
...
serve(async (req: Request) => {
  ...
  const donneursCompatibles = (profils as ProfilDonneur[]).filter((p) =>
    estCompatible(demande.groupe_sanguin_recherche, p.groupe_sanguin) &&
    estEligible(p)
  );

  if (donneursCompatibles.length === 0) {
    console.log("[matcher] Aucun donneur compatible pour", demande.groupe_sanguin_recherche);
    return jsonResponse({ matched: 0 }, 200, corsHeaders);
  }

  // ── Application du plafond MAX_DESTINATAIRES (ajouté 2026-07-16) ──────────
  const totalCompatibles = donneursCompatibles.length;
  const donneursFiltres = donneursCompatibles.slice(0, MAX_DESTINATAIRES);
  const nbExclusPlafond = totalCompatibles - donneursFiltres.length;

  if (nbExclusPlafond > 0) {
    console.log(
      `[matcher] Plafond atteint : ${totalCompatibles} donneurs compatibles, ` +
      `${donneursFiltres.length} notifiés (${nbExclusPlafond} non notifiés ce cycle).`,
    );
  }
